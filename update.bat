...
if not defined mode goto invalid_choice
...
if !errorlevel! NEQ 0 (
    echo [ОШИБКА] Не удалось переключиться на ветку !selected_branch!.
    pause
    goto :EOF
)
...
echo.
echo [ЗАГРУЗКА] Переключение на ветку !selected_branch!...
git checkout !selected_branch!
if !errorlevel! NEQ 0 (
    echo [ОШИБКА] Не удалось переключиться на ветку !selected_branch!.
    pause
...
if not defined commit_msg set "commit_msg=Auto-update from local script"
